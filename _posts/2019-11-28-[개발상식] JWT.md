...
예시를 조금 들면 아파치 톰캣의 경우 사용자의 세션을 컨테이너(톰캣 컨테이너)에 저장하게 된다. 여기서 확장성 있는 서비스를 만들기 위해 다수의 서버를 사용하게 되었을 때, 사용자의 세션이 정확히 어떤 컨테이너에 있는지 알 수 없다. 로그인을 분명 했는데 재 요청했을 때 다른 컨테이너로 들어가게 되면서 세션이 없다고 판단할 수도 있다.

2) 세션을 저장하기 위한 메모리 / 저장소 필요

현재 서비스를 이용하고 있는 모든 유저의 세션을 보관하기 위해서는 서버에 그만큼의 저장할 공간이 필요하게 된다. 몇백 몇천의 사용자가 사용하고 있을 때 그 하나 하나의 세션을 저장하고 유지하려면 많은 비용이 든다.
